...
set_tool(normal)                          # 툴 무게 변경(충전건 잡을 경우, type = chademo, combo)
vision_socket()                             # vision 통신 연결
                                            # 이동 완료후 비젼에 데이터 보냄(?, 연결해제 준비 완료 신호(?))
...
set_tool(gun_type)                          # 툴 무게 변경(충전건 잡았을경우(chademo or combo), type = chademo, combo)
...
if robot_communication.PTOR_data[0] == '0x04'  # 클램프 데이터를 받음
    set_tool_digital_output(1, ON)          # 충전건 클램프 OFF                                            

set_tool(normal)                          # 툴 무게 변경(충전건 놓았을경우(normal), type = chademo, combo)
